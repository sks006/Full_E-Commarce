...
export function Customers() {
  ...
  const { items, status, selectedCustomer, filters } = useAppSelector((state) => state.customers);
          ...
          <DataTable
            ...
            columns={[
              ...
              },
              {
                ...
                cell: (customer) => format(new Date(customer.createdAt), 'MMM dd, yyyy')
              },
              {
                header: 'Actions',
                accessorKey: 'actions',
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              ...
              <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-base">{selectedCustomer.phone || 'N/A'}</p>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-muted-foreground mb-1">Gender</h3>
                  <p className="text-base">{selectedCustomer.gender || 'N/A'}</p>
                </div>
                <div>
                  <h3 className="text-sm font-medium text-muted-foreground mb-1">Customer Since</h3>
                  <p className="text-base">{format(new Date(selectedCustomer.createdAt), 'MMMM dd, yyyy')}</p>
                </div>
              </div>
            </div>
            ...
            <Tabs defaultValue="orders">
                  ...
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="p-4 flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-medium">Order #{i}</p>
                          <p className="text-sm text-muted-foreground">
                            {format(new Date(2025, 0, i), 'MMM dd, yyyy')}
                          </p>
                        </div>
                      </div>
                      <div>
                  ...
                  {[1, 2].map((i) => (
                    <Card key={i}>
                      <CardContent className="p-4">
                        ...
                        <p className="text-sm">123 Example St, Unit {i}</p>
              ...
              <TabsContent value="activity" className="space-y-4">
                <div className="space-y-4">
                  {/* Mock activity */}
                  {[
                    { icon: <ShoppingBag className="h-4 w-4" />, text: 'Placed an order', date: '2 days ago' },
                    { icon: <User className="h-4 w-4" />, text: 'Updated profile information', date: '1 week ago' },
                    { icon: <Mail className="h-4 w-4" />, text: 'Subscribed to newsletter', date: '2 weeks ago' },
                  ].map((activity, i) => (
